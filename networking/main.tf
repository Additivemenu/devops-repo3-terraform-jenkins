# networking module is the first module that we will create. This module will create a VPC, public and private subnets, and an internet gateway. 
# treat the terraform module as a function declaration that takes in some arguments and returns some outputs.


# these below variables were like function arguments declaration
# decalre the variables that we want to pass to the module (we actually pass them from main.tf in repo root path) ------------------------------------------------
variable "vpc_cidr" {}
variable "vpc_name" {}
...
# declare the outputs that we want to return from the module -------------------------------------------------------------------------------------------
...
# now that we are defining the module, we can define the resources that we want to create. ============================================
# ! Setup VPC (4h17min- 4h20min)
# ! tf synax: resource "resource_type" "resource_name" {...}
...
# ! Setup public subnet (4h20min- )
resource "aws_subnet" "dev_proj_1_public_subnets" {
  count             = length(var.cidr_public_subnet) # we provision cidr_public_subnet as an array in terraform.tfvars
  vpc_id            = aws_vpc.dev_proj_1_vpc_eu_central_1.id   # ! reference to the VPC created above, in which VPC we want to create the subnet
  cidr_block        = element(var.cidr_public_subnet, count.index)  # element(): we provision eu_availability_zone as an array in terraform.tfvars
